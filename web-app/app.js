...
app.get('/getEmployeeData', function(req, res) {
  res.sendFile(path.join(__dirname + '/public/getEmployeeData.html'));
});
...
//post call to add Employee To Data
app.post('/api/addEmployee', function(req, res) {

  //declare variables to retrieve from request
  var employeeId = req.body.accountnumber;
  var cardId = req.body.cardid;

  var attributes =[];
  for(var i =0 ;i< Object.keys(req.body).length;i++){
    if(Object.keys(req.body)[i] == 'cardid' || Object.keys(req.body)[i] =='accountnumber'){
      console.log(Object.keys(req.body)[i] != 'cardid' )
      console.log(Object.keys(req.body)[i]);
      
    }
    else{
      attributes.push(Object.keys(req.body)[i]);
    }
  }
  ...
  network.addEmployee(cardId, employeeId, attributes,req.body)
          .then((response) => {
...
//Post Call to get Employee Data
app.post('/api/getEmployeeData', function(req, res) {

  //declare variables to retrieve from request
  var employeeId = req.body[0];
  

  network.getEmployeeData(employeeId, employeeId)
          .then((response) => {
            //return error if error in response
            if (response.error != null) {
              res.json({
