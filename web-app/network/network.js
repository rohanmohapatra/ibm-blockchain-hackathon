...
module.exports = {
  ...
  */
 addEmployee: async function (cardId, employeeId, attributes, jsonPassed) {
  console.log(cardId);
    ...
    try {
      ...
      const contract = network.getContract('employeeverfificationfinal');
      
      let member = {};
      member.employeeId = employeeId;
      let data ={};
      for(var i=0;i<attributes.length;i++){
        data[attributes[i]] = jsonPassed[attributes[i]];
      }
      /*
      data.name = "\""+name+"\"";
      data.email = "\""+email+"\"";
      ...
      const createMemberResponse = await contract.submitTransaction('CreateEmployee', JSON.stringify(member));
  ...
  getEmployeeData: async function (cardId, employeeId) {
    // Create a new file system based wallet for managing identities.
    const walletPath = path.join(process.cwd(), '/wallet');
    const wallet = new FileSystemWallet(walletPath);
    console.log(`Wallet path: ${walletPath}`);
    ...
    try {
      // Create a new gateway for connecting to our peer node.
      const gateway2 = new Gateway();
      await gateway2.connect(ccp, { wallet, identity: cardId.toString(), discovery: gatewayDiscovery });
      ...
      // Submit the specified transaction.
      console.log('\nSubmit Create Member transaction.');
      const createMemberResponse = await contract.submitTransaction('GetEmployeeData', employeeId.toString());
      console.log('createMemberResponse: ');
      console.log(JSON.parse(createMemberResponse.toString()));
 ...
 registerMember: async function (cardId, accountNumber, firstName, lastName, email, phoneNumber) {
    ...
    try {
      ...
      // Get the network (channel) our contract is deployed to.
      const network = await gateway2.getNetwork('hackathon10');
